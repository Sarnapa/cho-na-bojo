...
- **No change to the reject flow**, the contact-reveal endpoint's shape, or the venue map.
  ...
  > **Amended 2026-09-12 during implementation review (F5).** The venue-map guardrail was narrowed during implementation: `EventCardViewData` in `app/ChoNaBojoApp/ViewModels/MapViewModel.cs:93-96,119-134` gained re-request support for a `Left` request (`IsRequestable` / `CanJoin` previously required `status is null`) plus `Removed` / `Cancelled` status labels. The venue listing is the only join surface, so without it Phase 4's re-request-after-leaving rule would be unreachable from the UI and manual criterion 6.5 could not pass. No other venue-map change was made.
...
**Contract**: in `TransitionJoinRequestAsync`'s `createIfMissing` branch, the existing-row path at `:680-689` currently short-circuits to a replay for *any* existing row. It must now branch on status: under the event lock, a `Left` row is revived to `Pending`, `CreatedUtc` is reset to the current request-attempt time, and **`UpdatedUtc` is reset to `NULL`** (required by `CK_EventJoinRequests_StatusUpdatedUtc`); a `Removed` row returns 409 `request_already_resolved`; a `Cancelled` row is unreachable because the event-status gate rejects first; `Pending` and `Accepted` keep today's replay behaviour. Treat `CreatedUtc` in the request DTO as the current attempt's creation time, not the immutable first-ever creation time. Add a dedicated `PushIntentFactory` path for the revived `JoinRequestCreated` notification whose key is `join-request:{requestId}:{(int)type}:{recipientUserId}:{createdUtc.UtcTicks}` using the same timestamp written to the row. Duplicate retries encounter the row as `Pending` under the same event lock and return the existing replay without creating another outbox occurrence.

> **Amended 2026-09-12 during implementation review (F4).** This item originally read "Do not change keys for initial joins or the three new lifecycle types." That rule was wrong and the implementation deliberately departs from it: `BuildEventKey` (`server/Push/PushIntentFactory.cs:230`) appends the attempt stamp to **every** join-request-derived key. A revived row replays the whole lifecycle under the same request id, so a join → accept → leave → re-join → accept → leave cycle would emit a second `ParticipantLeft` with an identical 4-part key and fail the unique index on `PushOutbox.EventKey` (`server/Data/ChoNaBojoContext.cs:474-475`) with a 500. The attempt stamp is required on all of them, not just the revived `JoinRequestCreated`.
...
- [x] 3.1 Solution builds: `dotnet build solutions/ChoNaBojo.slnx` — e0c35a6
- [x] 3.2 Organizer cancel returns 200 with Status Cancelled — e0c35a6
- [x] 3.3 Repeat cancel returns 200, 0 notified, no new outbox rows — e0c35a6
- [x] 3.4 Non-organizer cancel returns 404 `event_not_found` — e0c35a6
- [x] 3.5 Cancel on a past-end event returns 409 `event_ended` — e0c35a6

#### Manual

- [x] 3.6 Mixed pending/accepted roster yields one outbox row each, delivered within 30s — e0c35a6
- [x] 3.7 Join-request rows read Status 6 with `UpdatedUtc`; event reads Status 2 with `StatusChangedUtc` — e0c35a6
- [x] 3.8 Cancelled event absent from the venue listing for every account — e0c35a6
- [x] 3.9 Contacts return 404 for organizer and previously accepted participant — e0c35a6
- [x] 3.10 Joining a cancelled event returns 409 `event_cancelled` — e0c35a6

### Phase 4: Remove Participant and Leave Event Endpoints

#### Automated

- [x] 4.1 Solution builds: `dotnet build solutions/ChoNaBojo.slnx` — 6181a6f
- [x] 4.2 Organizer remove returns 200 with Status Removed — 6181a6f
- [x] 4.3 Repeat remove returns 409 `participant_not_accepted` — 6181a6f
- [x] 4.4 Non-organizer remove returns 404 `request_not_found` — 6181a6f
- [x] 4.5 Accepted participant leave returns 200 with Status Left — 6181a6f
- [x] 4.6 Pending requester leave returns 409 `participant_not_accepted` — 6181a6f
- [x] 4.7 Organizer leave returns 404 `request_not_found` — 6181a6f
- [x] 4.8 Remove and leave on a cancelled event return 409 `event_cancelled` — 6181a6f

#### Manual

- [x] 4.9 Removed participant notified within 30s and contact access revoked both ways — 6181a6f
- [x] 4.10 Leave notifies the organizer within 30s and frees the slot in the venue listing — 6181a6f
- [x] 4.11 Re-request after leave refreshes `CreatedUtc`, revives the row to Pending with NULL `UpdatedUtc`, and emits one attempt-keyed notification — 6181a6f
- [x] 4.12 Re-request after removal is refused — 6181a6f
- [x] 4.13 Concurrent remove and accept never exceed `ParticipantLimit` — 6181a6f

### Phase 5: Client — Organizer Lifecycle Actions

#### Automated

- [x] 5.1 Android app builds: `dotnet build app/ChoNaBojoApp -f net10.0-android` — 918c70a
- [x] 5.2 Solution builds: `dotnet build solutions/ChoNaBojo.slnx` — 918c70a

#### Manual

- [x] 5.3 Cancel confirm dialog; dismissal makes no network call — 918c70a
- [x] 5.4 Confirmed cancel updates the card, closes selected detail, and removes revealed contacts without a refetch — 918c70a
- [x] 5.5 Cancel hidden or disabled on an already cancelled or finished event — 918c70a
- [x] 5.6 Remove confirms, updates the row in place, and hides the button — 918c70a
- [x] 5.7 Server-side conflict surfaces its message and the queue reconciles — 918c70a
- [x] 5.8 Offline attempts show the network snackbar and leave the UI unchanged — 918c70a
- [x] 5.9 Double-tap does not issue two requests — 918c70a
