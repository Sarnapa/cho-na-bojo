...
- **Triage**: complete on 2026-09-11 — 6 fixed, 1 skipped. Post-triage re-verification: `dotnet build solutions/ChoNaBojo.slnx` → 0 errors (117 pre-existing `MVVMTK0045` WinUI-AOT warnings unchanged).

## Triage outcome

| Finding | Decision |
|---------|----------|
| F1 — FCM sends inside the outbox transaction | FIXED via Fix A |
| F2 — Device/state matrix never recorded | FIXED (deferral made visible; recorded as optional, not a release blocker) |
| F3 — Progress drops item 7.5 | FIXED (7.5 restored unchecked) |
| F4 — 7.3 checked without recorded measurements | FIXED (7.3 downgraded to unchecked) |
| F5 — Unobserved fire-and-forget tasks | FIXED |
| F6 — `PushIntentFactory` "zero or one" contract | SKIPPED (revisit in S-07) |
| F7 — Out-of-plan file changes | FIXED (plan `## Addenda` added) |

Post-triage, Phase 7 Progress honestly reports `7.3` and `7.5` as pending, so **Success Criteria remains FAIL by design** — the two outstanding items are tracked as optional additional verification in `context/foundation/todo.md`, not as release blockers.
...
- **Decision**: FIXED via Fix A — `PushOutboxProcessor` split into claim / send / finalize phases (`ClaimItemAsync`, `ProcessItemAsync`, `FinalizeItemAsync`). The claim transaction now writes a 2-minute lease into `NextAttemptUtc` before committing, which preserves the double-send guard once sends run outside the row lock; each delivery outcome is committed on its own `SaveChangesAsync(CancellationToken.None)`; and an unexpected `SendAsync` exception is converted to `PushSendOutcome.Retryable("UnexpectedSendFailure", null)` so the existing `MaxAttempts = 5` backoff and dead-lettering bound it. No schema change — the bounding counter is the existing durable `PushDelivery.AttemptCount`, which now commits per attempt. `dotnet build server` → 0 errors.
...
- **Decision**: FIXED — `7.5` restored as unchecked in the plan's Phase 7 Manual block. Per the user's direction the matrix is recorded as an **optional additional verification step, not a release blocker**: `context/foundation/todo.md` and `manual-verification.md` were reworded from "Before production release" to "Worth verifying when device time allows", and the "Deferred Release Follow-up" heading became "Deferred Follow-up".
...
- **Decision**: FIXED — `7.5` re-inserted as unchecked between 7.4 and 7.6 by the F2 edit; Phase 7 no longer reports 100%. `todo.md` wording changed from "former Progress item 7.5" to "Progress item 7.5 (restored as unchecked)".
...
- **Decision**: FIXED — `7.3` downgraded to `- [ ]` in the plan's Phase 7 Manual block with the evidence gap stated inline. `manual-verification.md` now distinguishes "confirmed under 30 seconds in observed use" from the uncaptured five recorded runs, and the runs were added as item 4 of the deferred verification list in both `manual-verification.md` and `context/foundation/todo.md`.
...
- **Decision**: FIXED — added a private `ObserveAsync(Func<Task>, string description)` helper that awaits the operation inside `try/catch` and logs failures with `Log.Warn(LogTag, ...)`. All three discard sites (`OnRegistered`, `OnMessageReceived`, `OnDeletedMessages`) now route through it with a distinct description. `dotnet build app/ChoNaBojoApp -f net10.0-android` → 0 errors.
...
- **Decision**: SKIPPED — accepted as-is for S-06; revisit when S-07 extends the factory.
...
- **Decision**: FIXED — added an `## Addenda` section to `plan.md` (between `## References` and `## Progress`) recording all eight out-of-plan files with their justification, the `LoginPage.xaml.cs` call-site drift, and the two post-review code changes from F1 and F5. No code change.
