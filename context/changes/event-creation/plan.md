...
- [x] 2.1 Solution builds with the event model — c13fce1
- [x] 2.2 EF resolves the context — c13fce1
- [x] 2.3 Migration is generated and listed — c13fce1
- [x] 2.4 Model and snapshot agree — c13fce1
- [x] 2.5 Idempotent SQL script contains the table, checks, foreign keys, and indexes — c13fce1

#### Manual

- [x] 2.6 Apply the migration through the Supabase session-mode 5432 connection and confirm runtime still uses transaction-mode 6543 — c13fce1
- [x] 2.7 Supabase schema inspection confirms all event constraints, foreign keys, and indexes — c13fce1
- [x] 2.8 Direct invalid inserts are rejected by the database constraints — c13fce1

### Phase 3: Authenticated Idempotent Create API

#### Automated

- [x] 3.1 Server and solution build — e861f8c
- [x] 3.2 API runs in the background on http://localhost:5100 and stays up for 3.3-3.8, which use a JWT from /auth/login — e861f8c
- [x] 3.3 Unauthenticated POST /api/events returns 401 and creates no row — e861f8c
- [x] 3.4 Valid authenticated request returns 201 with safe complete detail data — e861f8c
- [x] 3.5 Repeating the request id returns 200 with the same event and only one row — e861f8c
- [x] 3.6 Invalid and stale-reference requests return the agreed 400/409 outcomes without creating rows — e861f8c
- [x] 3.7 Normal valid create call completes within five seconds — e861f8c
- [x] 3.8 Sub-microsecond-precision timestamps round-trip consistently and replay returns the same event id — e861f8c

#### Manual

- [x] 3.9 Supabase row confirms JWT-derived organizer, normalized text, and UTC timestamps — e861f8c
- [x] 3.10 First-create and replay serialized response bodies contain no login or contact data — e861f8c

### Phase 4: MAUI Create Form and Recovery Flow

#### Automated

- [x] 4.1 Whole solution builds — d044a0f
- [x] 4.2 Android head builds — d044a0f
- [x] 4.3 Windows head builds — d044a0f
- [x] 4.4 Venue create-event placeholder is removed — d044a0f
...
- [x] 4.11 Stale venue refreshes the catalog, explains, closes the form, and returns to the map — d044a0f
- [x] 4.12 Hardware Back and swipe-dismiss are refused while a create request is in flight — d044a0f

### Phase 5: Created Event Detail and End-to-End Handoff

#### Automated

- [x] 5.1 Whole solution builds — 0f7fcd1
- [x] 5.2 Android and Windows heads build — 0f7fcd1
- [x] 5.3 Event detail privacy scan finds no contact binding — 0f7fcd1
- [x] 5.4 EF model remains synchronized after all source changes — 0f7fcd1

#### Manual

- [x] 5.5 Valid creation opens complete device-local detail within the five-second normal target — 0f7fcd1
- [x] 5.6 Detail is contact-free and Back/Close returns to the same map and venue without resubmission — 0f7fcd1
- [x] 5.7 Canceling an unsubmitted form returns to the venue and creates nothing — 0f7fcd1
- [x] 5.8 Expired-token refresh replays the complete protected POST body exactly once — 0f7fcd1
- [x] 5.9 Double-tap and response-loss retry produce exactly one event and correct feedback — 0f7fcd1
- [x] 5.10 App restart makes no detail-restoration promise while the event remains persisted for S-04 — 0f7fcd1
