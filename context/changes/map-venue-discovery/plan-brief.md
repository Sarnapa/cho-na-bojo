...
| Location denied | Center on Warsaw + banner + address box | The map is never broken or empty, and every venue is in Warsaw anyway. |
| Geocoding | Built-in `Geocoding.Default` only | Least code and no third-party dependency; the emulator caveat is handled by documenting the test setup. |
...
| 5. Filter + fallback | Chip row filtering and recoloring; address search + banner | Built-in geocoding returns empty on non-Google-APIs emulators, mimicking a bug |
...
- **Geocoding testing is environment-sensitive.** On an emulator without Google Play Services the address fallback returns empty results that look identical to "address not found".
